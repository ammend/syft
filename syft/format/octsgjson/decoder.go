...
func (d decoder) Decode(r io.Reader) (*sbom.SBOM, sbom.FormatID, string, error) {
	...
	if id != ID {
		return nil, "", "", fmt.Errorf("not a Open Chain Telco Guide json document")
	}
	if version == "" {
		return nil, "", "", fmt.Errorf("unsupported Open Chain Telco Guide json document version")
	}

	if _, err := reader.Seek(0, io.SeekStart); err != nil {
		return nil, "", "", fmt.Errorf("unable to seek to start of Open Chain Telco Guide JSON SBOM: %+v", err)
	}

	doc, err := spdxJson.Read(reader)
	if err != nil {
		return nil, id, version, fmt.Errorf("unable to decode Open Chain Telco Guide json: %w", err)
	}

	s, err := octsghelpers.ToSyftModel(doc)
	if err != nil {
...
func (d decoder) Identify(r io.Reader) (sbom.FormatID, string) {
	...
	if _, err := reader.Seek(0, io.SeekStart); err != nil {
		log.Debugf("unable to seek to start of Open Chain Telco Guide JSON SBOM: %+v", err)
		return "", ""
	}

	// Example JSON document
	// {
	// "spdxVersion": "octsg-1.0",
	// ...
...
func getFormatInfo(spdxVersion string) (sbom.FormatID, string) {
	...
	if !strings.HasPrefix(strings.ToLower(spdxVersion), "octsg-") {
		return "", ""
	}
